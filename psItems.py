...
class Array(Expr):
    ...
    #needs to be updated
    def evaluate(self,psstacks):
        initialOpstackCount = len(psstacks.opstack) # gets the initial number of items on the opStack
        newArray = [] # creates an empty array to store evaluated array contents

        # goes through each item in the Array object
        for item in self.value:
            item.evaluate(psstacks)

        # Now, pop the results and adds to the new aarray
        for i in range(initialOpstackCount, len(psstacks.opstack)):
            newArray.append(psstacks.opPop())
        
        # reverse array since it appended starting from the top of the stack
        newArray.reverse()
        
        # pushes the ArrayValue of the new array onto the stack
        psstacks.opPush(ArrayValue(newArray))
...
class Name(Expr):
    ...
    def evaluate(self,psstacks):
        # if it starts with a / then push the string name
        if self.var_name[0] == '/':
            psstacks.opPush(str(self.var_name))    
        elif self.var_name in psstacks.builtin_operators:
            # checks to see if the Name is a built-in operator and then perfomrs the operation
            psstacks.builtin_operators[self.var_name]()
        else:
            variableValue = psstacks.lookup(self.var_name) # looks up variable/codearray in dictionary
            if isinstance(variableValue, FunctionValue): # if its a functionvalue then it applies the function value
                staticLinkIndex = psstacks.calculateStaticLinkHelper(self.var_name,-1) # gets the staticLink
                psstacks.dictPush(staticLinkIndex, {}) # pushes new AR tuple to dictStack
                variableValue.apply(psstacks) # calls apply
                psstacks.dictPop() # pops the top AR in the dictStack
            else:
                psstacks.opPush(variableValue) # it pushes the value of a variable onto the opstack
...
class FunctionValue(Value):
    ...
    def apply(self, psstacks):
        # goes through the nested list of expressions and applies each expression
